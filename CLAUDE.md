...
> **AgentsDiscover Project Overrides** (override the generic template guidance below):
> - **Source of truth for design**: `../agentsdiscover-internal-docs/design-system/agentsdiscover/MASTER.md`
> - **Source of truth for product/data/API**: `../agentsdiscover-internal-docs/dev-doc.md` and `../agentsdiscover-internal-docs/brd.md`
> - **Component library policy** (overrides "use AntD by default" below):
>   - **Tailwind CSS 4 + hand-built primitives** for everything brand-bearing — Buttons, Inputs, Cards, Tags, Badges, StarRating, ListingCard, ReviewCard, SearchBar, FilterSidebar, Tabs, Avatar, Pagination, Tables.
>   - **Ant Design 6 ONLY for these high-effort interactive primitives**: `Drawer`, `Modal` (complex cases), `DatePicker`, `Tooltip`, `Popover`, `Notification`/`message`, `Upload`, `Tour`. Always per-component import. Always themed via `<ConfigProvider>` to red `#D32323` + Inter.
>   - **No FontAwesome** — replace with **Lucide React** (`lucide-react`). `@fortawesome/*` packages will be removed in Phase 1.
>   - **Brand red `#D32323`** is reserved for: logo, primary CTA, focus rings. Do NOT flood with red.
